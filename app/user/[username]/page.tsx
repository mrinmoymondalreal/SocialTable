...
async function isFollowing(id: number, self_id: number) {
  let resp =
    await sql`SELECT relationship_id from user_relationships WHERE follower_id = ${self_id} AND following_id = ${id} LIMIT 1`;
  return resp.rows[0];
}
...
export default async function Page({
  ...
  let self = user && ((await getNextServerSession())?.user as User);
